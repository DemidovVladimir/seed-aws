...
export interface IContestSeasonDeletedOrFinishedEvent {
  id: string
...
export type DetailType = 'CONTEST_SEASON_DELETED' | 'CONTEST_SEASON_FINISHED'
export type EventType = EventBridgeEvent<
  DetailType,
  IContestSeasonDeletedOrFinishedEvent
>

export const contestSeasonCreatedEventHandler = async ({
  detail,
}: EventType) => {
  logger.debug('contestSeasonDeletedOrFinishedEventHandler', { detail })
  ...
  const contestantIds = (await service.loadContestants(id)).items.map(
    (item) => item.id
  )

  if (contestantIds) {
    await service.removeContestantBatch(contestantIds)
  }

  await service.removeContest(id)
}
